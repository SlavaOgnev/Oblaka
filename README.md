...
На основе анализа сервисных данных, мы видим, что Яндекс облако хорошо подходит для работы с БД (+развертка MongoDB сервера), но полная миграции невозможна потому, что, например, у нас нет аналога сервиса, которые позволяет добавлять ускореное глубокое обучение. Яндекс облако хорошо подходт для анализа данных, разработки и эксплуатации моделей машинного обучения. Так же неплох для управления ВМ. Оданко не существует анолга позволяющий подбирать рекламу под пользователя.
